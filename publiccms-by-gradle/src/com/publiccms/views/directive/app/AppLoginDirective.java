    ...
    public void execute(RenderHandler handler, SysApp app, SysUser user) throws IOException, Exception {
        ...
        if (notEmpty(username) && notEmpty(password)) {
            ...
            if (user.getPassword().equals(encode(password))) {
                ...
                sysUserTokenService.save(new SysUserToken(user.getId(), authToken, app.getChannel(), getDate(), ip));
                service.updateLoginStatus(user.getId(), ip);
                logLoginService.save(new LogLogin(site.getId(), username, user.getId(), app.getChannel(), ip, true, getDate(), ip));
