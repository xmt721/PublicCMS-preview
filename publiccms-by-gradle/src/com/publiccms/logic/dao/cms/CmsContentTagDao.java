...
public class CmsContentTagDao extends BaseDao<CmsContentTag> {
    public PageHandler getPage(Long tagId, Long contentId, Long[] contentIds, Integer pageIndex, Integer pageSize) {
        ...
        if (notEmpty(tagId)) {
            queryHandler.condition("bean.id.tagId = :tagId").setParameter("tagId", tagId);
        }
        ...
        if (notEmpty(contentIds)) {
            queryHandler.condition("bean.id.contentId not in (:contentIds)").setParameter("contentIds", contentIds);
        }
    ...
    public int delete(Long[] tagIds, Long[] contentIds) {
        if (notEmpty(tagIds) || notEmpty(contentIds)) {
            ...
            if (notEmpty(tagIds)) {
                queryHandler.condition("bean.id.tagId in (:tagIds)").setParameter("tagIds", tagIds);
            }
            if (notEmpty(contentIds)) {
                queryHandler.condition("bean.id.contentId in (:contentIds)").setParameter("contentIds", contentIds);
            }
