...
public class ScheduledTask extends Base {
    ...
    @Autowired
    public void init(SysTaskService sysTaskService, SysSiteService siteService, Scheduler scheduler, LogTaskService logTaskService) {
    ...
    public void create(SysSite site, Integer id, String cronExpression) {
        if (notEmpty(id) && notEmpty(cronExpression)) {
            ...
            try {
                ...
                CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(site.getId() % 60 + " " + cronExpression);
                if (empty(trigger)) {
