...
public class SysDomainAdminController extends AbstractController {
    ...
    @RequestMapping("save")
    public String save(SysDomain entity, String domainName, HttpServletRequest request, HttpSession session, ModelMap model) {
        SysSite site = getSite(request);
        if (verifyCustom("noright", !siteComponent.isMaster(site.getId()), model)) {
            return TEMPLATE_ERROR;
        }
        if (notEmpty(domainName)) {
            if (!entity.getName().equals(domainName) && verifyHasExist("domain", service.getEntity(entity.getName()), model)) {
                return TEMPLATE_ERROR;
            }
            entity = service.update(entity.getName(), entity);
            if (notEmpty(entity)) {
    ...
    @ResponseBody
    public boolean virify(String name, String domainName, Integer id, ModelMap model) {
        if (notEmpty(name)) {
