<dl>
	<dt>
		<h3><a href="<#if a.onlyUrl>${site.dynamicPath}content/redirect.do?id=${a.id}<#else>${a.url!}</#if>" target="_blank">${a.title}</a><span>${a.publishDate?date}</span></h3>
	</dt>
	<dd>
...
<#if a.hasImages>
      <@_contentFileList contentId=a.id count=2>
		<#list page.list as i>
